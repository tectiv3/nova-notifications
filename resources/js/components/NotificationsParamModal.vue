<template>
    <modal v-if="selectedNotification">
        <div class="bg-white rounded-lg shadow-lg overflow-hidden" style="width: 600px;">
            <div class="p-8">
                <heading :level="1" class="mt-6 mb-6">{{
                    selectedNotification.name
                }}</heading>

                <!-- START Notifiable Select -->
                <heading v-if="!hideNotifiableSelector" :level="2" class="mt-6 mb-6">{{
                    __('Send Notification To')
                }}</heading>
                <div
                    class="md:flex md:items-center mb-6"
                    v-if="!hideNotifiableSelector && formObj.notifiable"
                >
                    <div class="md:w-1/3">
                        <label
                    ...
                    <div class="md:w-2/3">
                        <div class="relative">
                            <select
                                id="notifiable-item"
                                v-model="formObj.notifiable.name"
                            ...
                            >
                                <option
                                    v-for="option in notifiables.filter.options"
                                    :value="option.name"
                                >
                                    {{ option.name }}
                                </option>
                ...
                <div
                    class="md:flex md:items-center mb-6"
                    v-if="!hideNotifiableSelector && formObj.notifiable.name"
                >
                    <div class="md:w-1/3">
                        <label
                            class="block text-grey font-bold md:text-right mb-1 md:mb-0 pr-4"
                ...
                <heading
                    ...
                    class="mt-6 mb-6"
                    v-if="
                        !hideNotifiableSelector && selectedNotification.parameters.length
                    "
                >
                ...
                <div
                    class="md:flex md:items-center mb-6"
                    v-if="selectedNotification.parameters.length"
                    v-for="(param, index) in selectedNotification.parameters"
                >
                    <div v-if="!hideNotifiableSelector" class="md:w-1/3">
                        <label
                            class="block text-grey font-bold md:text-right mb-1 md:mb-0 pr-4"
                            for="notifiable-item"
                        >
...
export default {
    ...
    mounted() {
        if (this.selectedNotification.name == 'App\\Notifications\\Announcement') {
            this.formObj.notifiable.name = 'App.User'
            this.hideNotifiableSelector = true
        }
    },
}
</script>
